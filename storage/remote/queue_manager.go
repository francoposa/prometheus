...
func (t *QueueManager) sendMetadataWithBackoff(ctx context.Context, metadata []prompb.MetricMetadata, pBuf *gogoproto.Buffer) error {
...
func (s *shards) runShard(ctx context.Context, shardID int, queue *queue) {
	...
	for i := range pendingDataV2 {
		...
		//pendingDataV2[i].Metadata = &writev2.Metadata{}
	}
	...
	sendBatch := func(batch []timeSeries, protoMsg config.RemoteWriteProtoMsg, enc Compression, timer bool) {
		...
		case config.RemoteWriteProtoMsgV2:
			// TODO BUFFER
			nPendingSamples, nPendingExemplars, nPendingHistograms, nPendingMetadata := populateV2TimeSeries(&symbolTable, batch, pendingDataV2, nil, s.qm.sendExemplars, s.qm.sendNativeHistograms)
			n := nPendingSamples + nPendingExemplars + nPendingHistograms
...
func (s *shards) sendSamples(ctx context.Context, samples []prompb.TimeSeries, sampleCount, exemplarCount, histogramCount int, pBuf *gogoproto.Buffer, buf *[]byte, enc Compression) error {
	begin := time.Now()
...
func (s *shards) sendSamplesWithBackoff(ctx context.Context, samples []prompb.TimeSeries, sampleCount, exemplarCount, histogramCount, metadataCount int, pBuf *gogoproto.Buffer, buf *[]byte, enc Compression) (WriteResponseStats, error) {
...
func (s *shards) sendV2SamplesWithBackoff(ctx context.Context, samples []*writev2.TimeSeries, labels []string, sampleCount, exemplarCount, histogramCount, metadataCount int, pBuf, buf *[]byte, enc Compression) (WriteResponseStats, error) {
	// Build the WriteRequest with no metadata.
	req, highest, lowest, err := buildV2WriteRequest(s.qm.logger, samples, nil, labels, pBuf, buf, nil, enc) // TODO BUFFER
	s.qm.buildRequestLimitTimestamp.Store(lowest)
	if err != nil {
		// Failing to build the write request is non-recoverable, since it will
		// only error if marshaling the proto to bytes fails.
...
func populateV2TimeSeries(symbolTable *writev2.SymbolsTable, batch []timeSeries, pendingData []*writev2.TimeSeries, sampleBuf chan *writev2.Sample, sendExemplars, sendNativeHistograms bool) (int, int, int, int) {
	var nPendingSamples, nPendingExemplars, nPendingHistograms, nPendingMetadata int
	for nPending, d := range batch {
		pendingData[nPending].Samples = pendingData[nPending].Samples[:0]
		if d.metadata != nil {
			//if pendingData[nPending].Metadata == nil {
		...
		// retries endlessly, so once we reach max samples, if we can never send to the endpoint we'll
		// stop reading from the queue. This makes it safe to reference pendingSamples by index.
		//labelsRefs := symbolTable.SymbolizeLabels(d.seriesLabels, nil)
		//pendingData[nPending].LabelsRefs = append(pendingData[nPending].LabelsRefs, labelsRefs...)
		pendingData[nPending].LabelsRefs = symbolTable.SymbolizeLabels(d.seriesLabels, pendingData[nPending].LabelsRefs)
		switch d.sType {
		case tSample:
			nPendingSamples = appendSample(pendingData, sampleBuf, nPending, d, nPendingSamples)
		case tExemplar:
...
func appendSample(pendingData []*writev2.TimeSeries, sampleBuf chan *writev2.Sample, nPending int, d timeSeries, nPendingSamples int) int {
	sample := <-sampleBuf
...
func buildWriteRequest(logger *slog.Logger, timeSeries []prompb.TimeSeries, metadata []prompb.MetricMetadata, pBuf *gogoproto.Buffer, buf *[]byte, filter func(prompb.TimeSeries) bool, enc Compression) (compressed []byte, highest, lowest int64, _ error) {
	...
	if pBuf == nil {
		pBuf = gogoproto.NewBuffer(nil) // For convenience in tests. Not efficient.
	} else {
		pBuf.Reset()
	}
...
func buildV2WriteRequest(logger *slog.Logger, samples []*writev2.TimeSeries, sampleBuf chan *writev2.Sample, labels []string, pBuf, buf *[]byte, filter func(*writev2.TimeSeries) bool, enc Compression) (compressed []byte, highest, lowest int64, _ error) {
