...
func TestDecodeWriteV2Request(t *testing.T) {
	...
	inputTS := writeV2RequestFixture.CloneVT()

	buf, _, _, err := buildV2WriteRequest(promslog.NewNopLogger(), inputTS.Timeseries, writeV2RequestFixture.Symbols, nil, nil, nil, "snappy")
	require.NoError(t, err)

	actual, err := DecodeWriteV2Request(bytes.NewReader(buf))
	require.NoError(t, err)
	require.Equal(t, writeV2RequestFixture.Timeseries, actual.Timeseries)
}
