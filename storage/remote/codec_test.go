...
func TestDecodeWriteV2Request(t *testing.T) {
	...
	buf, _, _, err := buildV2WriteRequest(promslog.NewNopLogger(), timeSeries, nil, writeV2RequestFixture.Symbols, nil, nil, nil, "snappy")
	require.NoError(t, err)

	actual, err := DecodeWriteV2Request(bytes.NewReader(buf))
	require.NoError(t, err)
	require.EqualExportedValues(t, writeV2RequestFixture, actual)
}
